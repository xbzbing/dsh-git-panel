...
async function runQuery(deps, config, request) {
  ...
  if (!workspace.ok) {
    ...
    return {
      ok: false,
      error: {
        code: error.code === "not-a-git-repo" || error.code === "cwd-unavailable" || error.code === "session-not-found" || error.code === "timeout" || error.code === "cancelled" || error.code === "git-unavailable" ? error.code : "git-error",
