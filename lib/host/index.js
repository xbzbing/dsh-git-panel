...
async function runQuery(deps, config, request) {
  ...
  try {
    switch (q.kind) {
      ...
      case "dir-list":
        return await queryDirList(deps, root, q);
      case "file-content":
        return await queryFileContent(deps, config, root, q);
      case "show":
...
async function isInsideRoot(deps, root, path) {
  try {
    const real = await deps.fs.realpath(path);
    const rootReal = await deps.fs.realpath(root);
    return real === rootReal || real.startsWith(rootReal + sep2);
  } catch {
    return false;
  }
}
